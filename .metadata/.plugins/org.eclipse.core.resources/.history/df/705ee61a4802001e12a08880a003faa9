...
<html>
    ...
    <body>
    	<br>
    	<h1 align = center>후보 삭제 완료</h1>
    	<form method = 'post'>	
			<table cellspacing="1" width="400" border="0" align="center" style="border-collapse: collapse;">
				<tr>
					<td width = 300></td>
					<td align="right">
	  				<input class="fourth" type="submit" value="후보 등록" style="width: 70px; height: 40px; padding: 0px;font-weight: bold;"formaction = 'updateForm.html'></td>
					<td align="right">
	  				<input class="fourth" type="submit" value="뒤로 가기" style="width: 70px; height: 40px; padding: 0px;font-weight: bold;"formaction = 'showREC.jsp'></td>
				</tr>
		</form>	
			</table>
 <%      
        ...
        int searchID = request.getParameter("tupyoID") != null ? Integer.parseInt(request.getParameter("tupyoID")) : 0;
        
        if(searchID != 0){
        String query = String.format("delete from TupyoDB where id = %d",searchID);        				
        stmt.execute(query);
        }
        ...
        while (rset1.next()){
 			score.add(rset1.getString(1)+"\t"+Integer.toString(rset1.getInt(2)));
 		}
